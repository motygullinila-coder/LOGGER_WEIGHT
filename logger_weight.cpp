...
volatile bool logger_running = false; // -> флаг синхронизации логирования данных
  ...
  // -> GUI
    ...
    void update_status() {
      if (logger_running) {
        ...
        lv_obj_set_style_bg_color(icon_status_label, lv_palette_main(LV_PALETTE_GREEN), LV_PART_MAIN);
      } else {
        lv_label_set_text(status_label, "STOP");
        lv_obj_set_style_bg_color(icon_status_label, lv_palette_main(LV_PALETTE_RED), LV_PART_MAIN);
      }
    }
  // -> GUI
